...
import { BatchNamingRulesCard } from "@/components/classes/BatchNamingRulesCard";
import { PreYouthBatchList } from "@/components/classes/PreYouthBatchList";
import { PreYouthBatchModal } from "@/components/modal/pre-youth-batch";
import { BaptismalBatchList } from "@/components/classes/BaptismalBatchList";
import { ETSBatchList } from "@/components/classes/ETSBatchList";
import {
  GraduationCap,
  Plus,
  Users,
...
export default function ClassManagementOverview({
  onNavigateToPreYouthDetail,
  onNavigateToBaptismalDetail,
  onNavigateToETSDetail,
}: ClassManagementOverviewProps = {}) {
  ...
  const handleViewBatch = (batch: Batch) => {
    ...
    if (activeTab === "pre-youth" && onNavigateToPreYouthDetail) {
      onNavigateToPreYouthDetail();
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        {activeTab === "pre-youth" ? (
          ...
          <BaptismalBatchList onViewBatch={handleViewBatch} />
        ) : activeTab === "ets" ? (
          /* ETS: Use specialized January/August sections */
          <ETSBatchList onViewBatch={handleViewBatch} />
        ) : (
                        ...
                        <div>
                          ...
                          <div className="w-full h-2 bg-gray-100 rounded-full overflow-hidden">
                            <div
                              ...
                              style={{
                                width: `${Math.min((batch.membersCount / batch.capacity) * 100, 100)}%`,
                              }}
                            />
                          </div>
                        </div>
                  ...
                  <CardContent className="p-12">
                    <div className="flex flex-col items-center justify-center text-center">
                      ...
                      <p className="text-sm text-[#8F9BB3] mb-4 max-w-md">
                        {canCreateBatch(activeTab)
                          ? `No ${getClassTypeLabel(activeTab).toLowerCase()} batches have been created yet. Create your first batch to get started.`
                          : `No ${getClassTypeLabel(activeTab).toLowerCase()} batches available. Batches are created automatically.`}
                      </p>
                      {canCreateBatch(activeTab) && (
                        <Button
                          onClick={handleCreateBatch}
      ...
      <PreYouthBatchModal
        ...
        onSave={(batchData) => {
          ...
          alert(
            `Pre-Youth Batch ${preYouthModalMode === "create" ? "created" : "updated"} successfully: ${batchData.batchName}`,
          );
          setShowPreYouthModal(false);
          setEditingBatch(null);
        }}
