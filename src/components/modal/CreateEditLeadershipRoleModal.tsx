...
interface CreateEditLeadershipRoleModalProps {
  ...
  roleData?: {
    id: string;
    roleName: string;
    category: 'SIC' | 'Band' | 'Unit' | 'Class' | 'Committee';
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-semibold text-[#222B45]">
                {mode === 'create' ? 'Create Leadership Role' : 'Edit Leadership Role'}
              </h2>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="md:col-span-2">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
                    {(['SIC', 'Band', 'Unit', 'Class', 'Committee'] as const).map((category) => (
                      <label
                        key={category}
                      ...
                      >
                        ...
                        <span className="text-sm font-medium text-[#222B45]">{category}</span>
                      </label>
                    ))}
                  </div>
                </div>
                ...
                <div className="md:col-span-2">
                  <label className="block text-sm font-medium text-[#222B45] mb-2">
                    Role Description <span className="text-[#8F9BB3] text-xs">(Optional)</span>
                  </label>
                  <textarea
                ...
                >
                  ...
                  <div className="ml-3 flex-1">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-medium text-[#222B45]">Global - Single Holder</span>
                      {formData.scopeType === 'Global' && (
                        <CheckCircle className="w-4 h-4 text-[#009AF4]" />
                      )}
                    </div>
                    <p className="text-xs text-[#8F9BB3] mt-1">
                      Only <span className="font-semibold">one person</span> can hold this role across the entire organization. 
                      Perfect for positions like National Overseer, Church Secretary, or Regional Pastor.
                    </p>
                    {formData.scopeType === 'Global' && formData.category === 'SIC' && (
                      <Card className="border-purple-200 bg-purple-50 mt-3">
                        <CardContent className="p-3">
                          <div className="flex items-start gap-2">
                            <Info className="w-4 h-4 text-purple-600 flex-shrink-0 mt-0.5" />
                            <p className="text-xs text-purple-900">
                              <span className="font-medium">SIC roles enforce single assignment:</span> Only one member can be 
                              assigned to this role at any time across the entire church.
                            </p>
                          </div>
                        </CardContent>
                      </Card>
                ...
                >
                  ...
                  <div className="ml-3 flex-1">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-medium text-[#222B45]">Contextual - Per Band/Unit</span>
                      {formData.scopeType === 'Contextual' && (
                        <CheckCircle className="w-4 h-4 text-[#009AF4]" />
                      )}
                    </div>
                    <p className="text-xs text-[#8F9BB3] mt-1">
                      This role can be assigned to <span className="font-semibold">one person per band, unit, class, or committee</span>. 
                      Perfect for Band Leader, Head of Unit, or Class Teacher where each context needs its own leader.
                    </p>
                    {formData.scopeType === 'Contextual' && (
                        ...
                        <CardContent className="p-3">
                          <div className="flex items-start gap-2">
                            ...
                            <p className="text-xs text-blue-900">
                              <span className="font-medium">Contextual roles require target selection:</span> When assigning this role, 
                              you'll need to specify which {formData.category.toLowerCase()} the person will lead.
                            </p>
                          </div>
                        </CardContent>
                      </Card>
              ...
              <div className="space-y-4">
                ...
                <div>
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <button
                      type="button"
                      onClick={() => handleInputChange('requiresTenure', !formData.requiresTenure)}
                    ...
                    >
                      <span
                        className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
                          formData.requiresTenure ? 'translate-x-6' : 'translate-x-1'
                        }`}
                      />
                    </button>
                  </div>
                    ...
                    <div className="grid grid-cols-2 gap-3 p-4 bg-[#F7F9FC] border border-[#EDF1F7] rounded-lg">
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) => handleInputChange('defaultTenureDuration', parseInt(e.target.value) || undefined)}
                          ...
                          className={`w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-[#009AF4] focus:border-transparent text-sm ${
                            touched.defaultTenureDuration && errors.defaultTenureDuration
                              ? 'border-red-300 bg-red-50'
                              : 'border-[#EDF1F7] bg-white'
                          }`}
                        />
                            ...
                            <CardContent className="p-3">
                              <div className="flex items-start gap-2">
                                ...
                                <p className="text-xs text-green-900">
                                  Default tenure: <span className="font-semibold">{formData.defaultTenureDuration} {formData.tenureUnit}</span>
                                  {formData.tenureUnit === 'months' && formData.defaultTenureDuration >= 12 && (
                                    <span className="text-green-800"> ({Math.floor(formData.defaultTenureDuration / 12)} year{Math.floor(formData.defaultTenureDuration / 12) > 1 ? 's' : ''})</span>
                                  )}
                                </p>
                              </div>
                            </CardContent>
                      ...
                      <CardContent className="p-3">
                        <div className="flex items-start gap-2">
                          ...
                          <p className="text-xs text-gray-800">
                            No tenure duration required. Assignments will have no automatic expiry date.
                          </p>
                        </div>
                      </CardContent>
                    </Card>
                ...
                <div className="flex items-center justify-between p-4 bg-[#F7F9FC] border border-[#EDF1F7] rounded-lg">
                  <div className="flex-1">
                    <p className="font-medium text-[#222B45] text-sm">Can Hold Multiple Roles</p>
                    <p className="text-xs text-[#8F9BB3] mt-0.5">
                  ...
                  <button
                    type="button"
                    onClick={() => handleInputChange('canHoldMultipleRoles', !formData.canHoldMultipleRoles)}
                    className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ml-4 ${
                      formData.canHoldMultipleRoles ? 'bg-[#009AF4]' : 'bg-gray-300'
                    }`}
                  >
                    <span
                      className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
                        formData.canHoldMultipleRoles ? 'translate-x-6' : 'translate-x-1'
                      }`}
                    />
                  </button>
                </div>
                    ...
                    <CardContent className="p-3">
                      <div className="flex items-start gap-2">
                        ...
                        <p className="text-xs text-orange-900">
                          <span className="font-medium">Multiple role assignment enabled:</span> When assigning this role, 
                          the system will allow members who already hold other leadership positions. Ensure this aligns with 
                          your church's governance policies.
                        </p>
                      </div>
                    </CardContent>
                  </Card>
