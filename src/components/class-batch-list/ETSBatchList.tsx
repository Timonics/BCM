...
export function ETSBatchList({ onViewBatch }: ETSBatchListProps = {}) {
  ...
  const uniqueYears = Array.from(new Set(batches.map((b) => b.year))).sort((a, b) => b - a);
  ...
  const renderBatchCard = (batch: ETSBatch) => (
    <Card key={batch.id} className="border-[#EDF1F7] shadow-sm hover:shadow-md transition-shadow">
        ...
        <div className="space-y-4">
            ...
            <div className="flex items-start gap-3 flex-1">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  <h3 className="text-lg font-semibold text-[#222B45]">{batch.batchName}</h3>
                  {batch.isAutoCreated && (
            ...
            <div className="flex-1">
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-medium text-[#222B45]">Members Enrolled</span>
                </div>
              ...
              <div className="w-full h-2 bg-gray-100 rounded-full overflow-hidden">
                <div
                  ...
                  style={{ width: `${(batch.membersCount / batch.capacity) * 100}%` }}
                />
              </div>
            </div>
          </div>
            ...
            <div className="p-3 bg-orange-50 border border-orange-200 rounded-lg">
              <div className="flex items-center justify-between mb-1">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-medium text-orange-900">Pending Approvals</span>
                </div>
              </div>
              <p className="text-2xl font-semibold text-orange-700">{batch.pendingApprovals}</p>
              <p className="text-xs text-orange-600 mt-1">
                {batch.pendingApprovals > 0 ? 'Awaiting review' : 'All processed'}
              </p>
            </div>
            ...
            <div className="p-3 bg-teal-50 border border-teal-200 rounded-lg">
              <div className="flex items-center justify-between mb-1">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-medium text-teal-900">Band Eligible</span>
                </div>
              </div>
              <p className="text-2xl font-semibold text-teal-700">{batch.bandEligibleCount}</p>
            ...
            <div className="flex-1">
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-medium text-[#222B45]">Completion Progress</span>
                </div>
                <span className="text-sm font-semibold text-[#222B45]">{batch.completionRate}%</span>
              </div>
              <div className="w-full h-2 bg-gray-100 rounded-full overflow-hidden">
                <div
                  className={`h-full transition-all ${
          ...
          <div className="flex items-center justify-between pt-3 border-t border-[#EDF1F7]">
            <div className="flex items-center gap-2 flex-wrap">
              <Badge variant="outline" className={getStatusColor(batch.completionStatus)}>
              ...
              {batch.hasLeadership ? (
                <Badge variant="outline" className="bg-green-50 text-green-700 border-green-200">
                  ...
                  {batch.leadershipCount} {batch.leadershipCount === 1 ? 'Leader' : 'Leaders'}
                </Badge>
              ) : (
  ...
  const renderEmptyState = (month: string) => (
    <Card className="border-[#EDF1F7] shadow-sm">
      <CardContent className="p-12 text-center">
        ...
        <h3 className="text-lg font-semibold text-[#222B45] mb-2">No {month} Batches Found</h3>
    ...
    <div className="space-y-6">
      ...
      <div className="p-4 bg-teal-50 border border-teal-200 rounded-lg">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            <h4 className="text-sm font-semibold text-teal-900 mb-1">System-Managed Batches</h4>
            <p className="text-xs text-teal-700">
              ETS (Establishment) Class batches are automatically created by the system for January and August intake
              periods. Batches cannot be deleted but leadership and member assignments can be managed. Track band
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <select
            ...
            className="px-4 py-2.5 border border-[#EDF1F7] rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#009AF4] bg-white min-w-[140px]"
          >
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-semibold text-[#222B45]">January Batches</h2>
              <p className="text-sm text-[#8F9BB3]">First semester intake (January - June)</p>
            </div>
          </div>
          <Badge variant="outline" className="bg-teal-50 text-teal-700 border-teal-200">
            {januaryBatches.length} {januaryBatches.length === 1 ? 'Batch' : 'Batches'}
          </Badge>
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          {januaryBatches.length > 0 ? januaryBatches.map(renderBatchCard) : renderEmptyState('January')}
        </div>
      </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-semibold text-[#222B45]">August Batches</h2>
              <p className="text-sm text-[#8F9BB3]">Second semester intake (August - January)</p>
            </div>
          </div>
          <Badge variant="outline" className="bg-emerald-50 text-emerald-700 border-emerald-200">
            {augustBatches.length} {augustBatches.length === 1 ? 'Batch' : 'Batches'}
          </Badge>
        </div>

        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          {augustBatches.length > 0 ? augustBatches.map(renderBatchCard) : renderEmptyState('August')}
        </div>
      </div>
    </div>
  );
