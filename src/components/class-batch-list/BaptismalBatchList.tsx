...
interface BaptismalBatchListProps {
  onViewBatch?: (batch: any) => void;
}

export function BaptismalBatchList({ onViewBatch }: BaptismalBatchListProps = {}) {
  const [openMenuId, setOpenMenuId] = useState<string | null>(null);
  const [searchQuery, setSearchQuery] = useState('');
  const [yearFilter, setYearFilter] = useState<string>('all');

  // Mock baptismal batches data
  const batches: BaptismalBatch[] = [
    // January 2026
    {
      id: 'bc1',
      batchName: 'BCJAN2026',
  ...
  const getStatusColor = (status: BaptismalBatch['completionStatus']) => {
    switch (status) {
      case 'Completed':
  ...
  const getStatusIcon = (status: BaptismalBatch['completionStatus']) => {
    switch (status) {
      case 'Completed':
  ...
  const handleViewBatch = (batch: BaptismalBatch) => {
    ...
    <div className="space-y-6">
      ...
      <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            <h4 className="text-sm font-semibold text-blue-900 mb-1">System-Managed Batches</h4>
            <p className="text-xs text-blue-700">
              Baptismal Class batches are automatically created by the system for January and August intake periods.
              Batches cannot be deleted but leadership and member assignments can be managed.
            </p>
          </div>
        </div>
      </div>
